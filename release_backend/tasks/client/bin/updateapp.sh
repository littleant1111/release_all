#!/bin/bash
# Filename: updateapp.sh
# Revision: 1.1
# Date: 2017/02/15
# Email: sheng.huang@bqjr.cn
# Usage: bash updateapp.sh
# 功能：更新war包到容器中 
# 详细如下：
#     通过判断容器类型，tomcat容器会删除缓存文件夹temp和work中的文件，并删除容器内代码，将更新包传送到容器中。
...
main(){
    ...
    if [ ${apptype[1]} == "tomcat" ];then
        ...
        # 更新应用程序包
        log_echo "[info]" "Start update application"
        typeset mvBackupdir="${TEMP_DIR}/${projectname}_${version}/${projectname}_${version}_${localTime}_update"
        upgradeTomcatApply ${appwarname} ${pkgfile} ${user[1]} ${appdir[1]} ${mvBackupdir} || return 1
    ...
    log_echo "[info]" "更新应用程序包成功 || Update the application package successed"
    return 0 
}
