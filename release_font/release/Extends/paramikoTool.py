...
class ParamikoTool(object):
    ...
    def execCommandInRemoteHost(self, ip, username, port, password, *cmdlist):
        ...
        try:
            ...
            for i in range(len(cmdlist)):
                ...
                #print 'sfasfsdsadffasfddfsa',type(status),status
                if status == 1:
                    self.returnresult += 1
                    return 'false'
                ...
                try:
                    ...
                    print "=========== 错误信息 3：",e
                    # self.returnresult += 1
                    print str(ip), int(port), str(username), str(password),cmdlist
                    return 'true'
            ssh.close()
        except Exception, e:
            print "=========== 错误信息 4：" + str(e)
    ...
    def remoteHostHasDir(self, ip, username, sshport, password, dirname):
        try:
            ...
            print ip, username, sshport, password, dirname
    ...
    def sftp_get_dir(self, host, username, port, password, local_dir, remote_dir):
        print  host, username, port, password, local_dir, remote_dir
